...
{   using half_float::half;
	...
	for (int rowTile = 0; rowTile < para.rowScale; rowTile++)
	{
		for (int colTile = 0; colTile< para.colScale; colTile++)
		{
			...
			tmm_kernel<<<grid,block>>>(para, model->gridSizeM, model->gridSizeN, model->k, gpuHalfp, gpuHalfq, prob->gpuR);
...
void multiplication(string test_path, const char* model_path)
{
    ...
    //core
    float **Rp = new float[para.rowScale*para.colScale][model->gridSizeM*model->gridSizeN];
    tmm_update_k128(Parameter para, tmm_model *model, tmm_problem *prob, float **Rp);
